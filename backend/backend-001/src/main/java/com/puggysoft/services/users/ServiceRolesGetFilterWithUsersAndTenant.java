...
@Service
public class ServiceRolesGetFilterWithUsersAndTenant {
  ...
  public ResponseEntity<List<DtoRole>> filter(
    ...
    if (query.equals("")) {
      listEntities = repositoryRole.findRolesWithUserAndTenant(idUser, off, size, tenant);
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT users_roles.id, users_roles.tenant as aux, roles.* FROM roles "
