...
@Service
public class ServiceRequisitosGetFilterWithoutTramitesSize {
  ...
  public ResponseEntity<Long> getSize(DtoUrbanismoRequisitosFilter dtoUrbanismoRequisitosFilter, Long pageSize, String tramite) {
    ...
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryRequisitos.findSizeWithoutTramites(tramite);
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM alc_urb_requisitos WHERE "
          + "alc_urb_requisitos.nombre_corto "
          + "NOT IN ( SELECT alc_urb_tramite_requisitos.requisito_nombre_corto "
