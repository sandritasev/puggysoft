...
@Service
public class ServiceEstadosGetFilterWithTramites {
  ...
  public ResponseEntity<List<DtoUrbanismoEstados>> filter(DtoUrbanismoEstadosFilter dtoUrbanismoEstadosFilter, int page, int size, String tramite) {
    ...
    if (query.equals("")) {
      listEntities = repositoryEstados.findEstadosWithTramites(tramite, off, size);
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT alc_urb_tramite_flujo.id, alc_urb_estados.* FROM alc_urb_estados "
          + "INNER JOIN alc_urb_tramite_flujo ON "
          + "alc_urb_tramite_flujo.estado_nombre_corto=alc_urb_estados.nombre_corto "
          + "WHERE alc_urb_tramite_flujo.tramite_nombre_corto = '" + tramite + "' AND "
