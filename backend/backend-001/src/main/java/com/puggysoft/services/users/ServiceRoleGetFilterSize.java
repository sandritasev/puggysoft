...
@Service
public class ServiceRoleGetFilterSize {
  ...
  /** method for get size. */
  public ResponseEntity<Long> getSize(DtoRoleFilter dtoRoleFilter, Long pageSize) {

    String query = SqlRoleFilterBuilderNative.build(dtoRoleFilter);
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryRole.findSize();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM roles WHERE " + query;
