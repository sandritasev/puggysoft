...
@Service
public class ServiceTenantGetFilterSize {
  ...
  /** method for get size. */
  public ResponseEntity<Long> getSize(DtoTenantFilter dtoTenantFilter, Long pageSize) {

    String query = SqlTenantFilterBuilderNative.build(dtoTenantFilter);
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryTenant.findSize();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM tenants WHERE " + query;
