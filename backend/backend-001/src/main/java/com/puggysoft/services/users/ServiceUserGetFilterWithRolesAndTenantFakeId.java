...
@Service
public class ServiceUserGetFilterWithRolesAndTenantFakeId {
  ...
  public ResponseEntity<List<DtoUser>> filter(
    ...
    if (query.equals("")) {
      listEntities = repositoryUser.findUsersWithRoleAndTenantFakeId(idRol, off, size, tenant);
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT users_roles.id, users.* FROM users "
