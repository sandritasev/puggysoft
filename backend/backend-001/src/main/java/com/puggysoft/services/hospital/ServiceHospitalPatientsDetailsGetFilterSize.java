...
@Service
public class ServiceHospitalPatientsDetailsGetFilterSize {
  ...
  /** method for get size. */
  public ResponseEntity<Long> getSize(DtoHospitalPatientsDetailsFilter dtoHospitalPatientsDetailsFilter,
      Long pageSize) {

    String query = SqlHospitalPatientsDetailsFilterBuilderNative.build(dtoHospitalPatientsDetailsFilter);
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryHospitalPatientsDetails.findSize();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM hospital_patients_details WHERE " + query;
