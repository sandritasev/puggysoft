...
@Service
public class ServiceAlcaldiaRecursosMunicipalesWithoutTimbresGetFilterSize {
  ...
  public ResponseEntity<Long> getSize(DtoAlcaldiaRecursosMunicipalesFilter dtoAlcaldiaRecursosMunicipalesFilter,
    ...
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryAlcaldiaRecursosMunicipales.findSizeOutTimbres();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM alcaldia_recursos_municipales WHERE name NOT LIKE \"%TIMBRES%\" AND " + query;
