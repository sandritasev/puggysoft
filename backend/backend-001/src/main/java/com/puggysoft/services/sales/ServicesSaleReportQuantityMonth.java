...
@Service
public class ServicesSaleReportQuantityMonth {

  @Autowired
  private IRepositorySaleProduct repositorySaleProduct;

  /** method for retrive. */
  public ResponseEntity<ArrayList<Long>> getReport(Integer year, Integer month, String tenant) {
    YearMonth yearMonthObject = YearMonth.of(year, month);
    int daysInMonth = yearMonthObject.lengthOfMonth();
    ArrayList<Long> arrayDays = new ArrayList<>();
    for (int day = 1; day <= daysInMonth; day++) {
      String date = year + "-" + month + "-" + day;
      Long sumDay = repositorySaleProduct.getQuantityPerDay(date, tenant);
