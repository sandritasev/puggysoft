...
@Service
public class ServiceEscuelaMateriasGetFilterSize {
  ...
  /** method for get size. */
  public ResponseEntity<Long> getSize(DtoEscuelaMateriasFilter dtoEscuelaMateriasFilter, Long pageSize) {

    String query = SqlEscuelaMateriasFilterBuilderNative.build(dtoEscuelaMateriasFilter);
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryEscuelaMaterias.findSize();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM escuela_materias WHERE " + query;
