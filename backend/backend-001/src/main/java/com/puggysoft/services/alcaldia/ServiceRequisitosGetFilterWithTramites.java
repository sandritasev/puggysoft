...
@Service
public class ServiceRequisitosGetFilterWithTramites {
  ...
  public ResponseEntity<List<DtoUrbanismoRequisitos>> filter(DtoUrbanismoRequisitosFilter dtoUrbanismoRequisitosFilter, int page, int size, String tramite) {
    ...
    if (query.equals("")) {
      listEntities = repositoryRequisitos.findRequisitosWithTramites(tramite, off, size);
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT alc_urb_tramite_requisitos.id, alc_urb_requisitos.* FROM alc_urb_requisitos "
          ...
          + "WHERE alc_urb_tramite_requisitos.tramite_nombre_corto = '" + tramite + "' AND "
