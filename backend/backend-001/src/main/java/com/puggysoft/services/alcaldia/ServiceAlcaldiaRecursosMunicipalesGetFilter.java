...
/** Services for filter. */
@Service
public class ServiceAlcaldiaRecursosMunicipalesGetFilter {
  ...
  public ResponseEntity<List<DtoAlcaldiaRecursosMunicipales>> filter(
    ...
    if (query.equals("")) {
      listEntities = repositoryAlcaldiaRecursosMunicipales.findAlcaldiaRecursosMunicipalesByPagination(off, size);
    } else {
