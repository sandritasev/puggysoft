...
@Service
public class ServiceStorageRecordGetFilterSize {
  ...
  /** method for get size. */
  public ResponseEntity<Long> getSize(DtoStorageRecordFilter dtoStorageRecordFilter,
      Long pageSize) {

    String query = SqlStorageRecordFilterBuilderNative.build(dtoStorageRecordFilter);
    Long totalRows = 0L;
    if (query.equals("")) {
      totalRows = repositoryStorageRecord.findSize();
    } else {
      // Delete last 'AND' key workd.
      query = query.substring(0, query.length() - 4);
      String fullQuery = "SELECT COUNT(*) FROM storage_record WHERE " + query;
