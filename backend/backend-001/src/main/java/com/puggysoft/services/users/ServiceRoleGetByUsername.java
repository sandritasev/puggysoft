...
@Service
public class ServiceRoleGetByUsername {

  @Autowired
  private IRepositoryRole repositoryRole;

  /** method for retrive. */
  public ResponseEntity<List<DtoRole>> getByUsername(String username) {
    List<DtoRole> listDtoRole = repositoryRole.findRolesByUsername(username)
            .stream()
