...
@Service
public class ServiceRoleGetFilterWithoutUser {
  ...
  public ResponseEntity<List<DtoRole>> filter(DtoRoleFilter dtoRoleFilter, int page, int size, Long iduser) {

    String query = SqlRoleFilterBuilderNative.build(dtoRoleFilter);
    int off = page * size;
    List<EntityRole> listEntities;
    if (query.equals("")) {
      listEntities = repositoryRole.findRolesWithoutUsersPagination(iduser, off, size);
    } else {
      ...
      String fullQuery = "SELECT roles.* FROM roles WHERE "
          ...
          + iduser + ") AND " + query + " LIMIT " + off + "," + size;
