...
function UsersRolesStepTwo () {
  ...
  function handleGetData (activePage, filterBody, updateArrayData) {
    handleFilterRequest(`roles/filter/without-users-and-tenants?page=${activePage - 1}&size=${pageSize}&idUser=${userData.id}&tenant=${tenant}`, filterBody, updateArrayData);
  }

  function handleGetSize (filterBody, setTotalPages) {
    handleFilterRequest(`roles/filter/without-users-and-tenants/size?pageSize=${pageSize}&idUser=${userData.id}&tenant=${tenant}`, filterBody, setTotalPages);
  }

  function handleGetDataTwo (activePage, filterBody, updateArrayData) {
    handleFilterRequest(`roles/filter/with-users-and-tenants?page=${activePage - 1}&size=${pageSize}&idUser=${userData.id}&tenant=${tenant}`, filterBody, updateArrayData);
  }

  function handleGetSizeTwo (filterBody, setTotalPages) {
    handleFilterRequest(`roles/filter/with-users-and-tenants/size?pageSize=${pageSize}&idUser=${userData.id}&tenant=${tenant}`, filterBody, setTotalPages);
  }
...
export default UsersRolesStepTwo;
