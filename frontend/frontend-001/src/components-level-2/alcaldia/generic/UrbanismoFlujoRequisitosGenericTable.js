...
import urbanismoFlujoRequisitosTableModels from "../../../models/alcaldia/urbanismoFlujoRequisitosTableModels";

function UrbanismoFlujoRequisitosGenericTable (props) {
  const {
    numberPagesToShow,
    tableTitle,
    tableSubTitle,
  ...
  const { value: criteriaEstado, onChange: criteriaOnChangeEstado, setValue: criteriaSetEstado } = useInput("");
  const { value: criteriaArchivo, onChange: criteriaOnChangeArchivo, setValue: criteriaSetArchivo } = useInput("");
  ...
  const { value: operatorEstado, onChange: operatorOnChangeEstado, setValue: operatorSetEstado } = useInput(enumCompareOperators.TEXT_CONTAINS);
  const { value: operatorArchivo, onChange: operatorOnChangeArchivo, setValue: operatorSetArchivo } = useInput(enumCompareOperators.TEXT_CONTAINS);
  ...
  const { arrayColumnsFilter, clearFilters, getFilterBody, arrayColumnsLabels, arrayDataFields } = urbanismoFlujoRequisitosTableModels(
    columnsToShow,
    ...
    /* ESTADO */criteriaEstado, criteriaOnChangeEstado, criteriaSetEstado, operatorEstado, operatorOnChangeEstado, operatorSetEstado,
    /* ARCHIVO */ criteriaArchivo, criteriaOnChangeArchivo, criteriaSetArchivo, operatorArchivo, operatorOnChangeArchivo, operatorSetArchivo,
...
export default UrbanismoFlujoRequisitosGenericTable;

UrbanismoFlujoRequisitosGenericTable.propTypes = {
...
UrbanismoFlujoRequisitosGenericTable.defaultProps = {
  numberPagesToShow: 0,
  tableTitle: "",
