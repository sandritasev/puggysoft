...
function SaleReportProfitMonthByProduct () {
  ...
  function handleUpdateData (year, month, setReportData, onRequestFail) {
    if (productData && productData.id) {
      ...
      handleGetRequest(`sales-report/profit-month-by-product?year=${year}&month=${month}&idProduct=${productData.id}&tenant=${tenant}`,
        setReportData,
        onRequestFail);
    } else {
  ...
  return (
    <>
      ...
      <ReportGenericByMonth
        ...
        pathNameOneOrTwoYears={enumPaths.SALES_REPORT_PROFIT_MONTH_COMP_BY_PRODUCT}
      />
    </>
  );
}

export default SaleReportProfitMonthByProduct;
